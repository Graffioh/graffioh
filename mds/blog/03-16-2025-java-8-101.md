...
They are **immutable** and **final**, so you can't modify its reference after the declaration.
...
- **Unchecked**: these are caused by the programmer (f.e. ```ArrayOutOfBoundsException```), the compiler don't complain if you don't handle them beforehand but causes runtime errors.
...
These two bounds also give important guarantees:
...
**Completeness** is when a method accept all possible valid inputs, whereas **Correctness** is when a method never accept invalid inputs.
...
That's useful for leetcode nerds that uses Java (I have no words for you).
