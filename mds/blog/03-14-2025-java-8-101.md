...
When using a custom `Comparator` with elements a, b, and c, and the `sgn()` function that returns the sign of the result, the `compare()` method must adhere to the following properties:
